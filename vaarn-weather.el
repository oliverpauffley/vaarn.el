;;; vaarn-weather.el --- Weather generation for overland travel -*- lexical-binding: t; -*-
;;
...
;;
;;; Commentary:
;; Weather generation using a hex flower for vaarn. This is done by storing
;; coordinates of the players and rolling a virtual dice to move around.
;; There are additional rules for where you can move to and wrapping around
;; flower to the otherside.
;;; Code:
...
(defvar vaarn-weather--starting-coord (make-cube-coord
                                       :q 0
                                       :r 0
                                       :s 0)
  "The start space on the weather grid (the centre).")

(define-multisession-variable vaarn-weather--current-coord nil
  "The current space on the weather grid."
  :package "vaarn"
  :synchronized t
  :storage 'sqlite)

(defun vaarn-weather-reset ()
  "Reset the weather to the centre."
  (interactive)
  (setf (multisession-value vaarn-weather--current-coord) vaarn-weather--starting-coord)
  (vaarn-weather-hex))
...
(defun vaarn-weather--draw-hex (location)
  ...
  (let* ((buf (get-buffer-create vaarn-weather--buffer-name))
    ...
    (save-excursion
      (with-current-buffer buf
        ...
        (insert (propertize weather-description 'face 'vaarn-weather--active-location))
        (goto-char 0)
  ...
  (mapcar (lambda (s)
            (let* ((sym (cdr s))
              ...
              (if (equal pos current-location)
                  (propertize hover-symbol 'face 'vaarn-weather--active-location)
                hover-symbol)))
          locations))
...
Either loads the location or sets to a default value."
  ...
  (let
    ...
    (display-buffer buf)
    (let ((coord (if (cube-coord-p (multisession-value vaarn-weather--current-coord))
                     (multisession-value vaarn-weather--current-coord)
                   vaarn-weather--starting-coord)))
      (vaarn-weather--draw-hex coord))))
...
Then use \\[vaarn-weather-move-hex] to move around the map.
